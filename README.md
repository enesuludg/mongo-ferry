...
`--dateField` does **not** sort by `_id` unless you pass `--sort '{"_id":1}'`. Sorting a date filter by `_id` on 3.6 can force an in-memory sort. `--resume` still requires `{_id:1}`. If the source has a **single-field** index on that date field (for example `{updatedAt:-1}`), mongo-ferry uses it automatically. `{updatedAt:1}` is a different key than `{updatedAt:-1}` — 3.6 `hint` must match exactly, or use `--hint updatedAtReverse`. Compound indexes that merely *contain* `updatedAt` are not used. Throughput is usually limited by fetching full documents from the source, not `--concurrency`; `--projection` and running near the source help more. `--onConflict skip` inserts only missing `_id`s and will not refresh already-copied docs.
